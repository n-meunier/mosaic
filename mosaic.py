...
TILE_SIZE = 600  # height/width of mosaic tiles in pixels
TILE_MATCH_RES = (
    5  # tile matching resolution (higher values give better fit but require more processing)
)
ENLARGEMENT = 18  # the mosaic image will be this many times wider and taller than the original
DIFF_RANGE = 500000  # Matching difference between the best match and the other allowed tiles
TRANSPARENCY = 200  # Define the transparency level (0 for fully transparent, 255 for fully opaque)
...
OUT_FILE = f"mosaic_tile-{TILE_SIZE}_transparency-{TRANSPARENCY}_diff-{DIFF_RANGE}_size-{ENLARGEMENT}_{NOW}.jpeg"
    ...
    def __process_tile(self, tile_path):
        try:
            ...
            small_tile_img = img.resize(
                (int(TILE_SIZE / TILE_BLOCK_SIZE), int(TILE_SIZE / TILE_BLOCK_SIZE)), Image.LANCZOS
            )
...
class TileFitter:
    ...
    def __get_tile_diff(self, t1, t2, bail_out_value):
        ...
        for i in range(len(t1)):
            ...
            diff += (
                (t1[i][0] - t2[i][0]) ** 2 + (t1[i][1] - t2[i][1]) ** 2 + (t1[i][2] - t2[i][2]) ** 2
            )
    ...
    def get_best_fit_tile(self, img_data):
        ...
        for tile_data in self.tiles_data:
            ...
            diff_scores[tile_index] = diff
            if diff < min_diff:
                min_diff = diff
                best_fit_tile_index = tile_index
            tile_index += 1
        # ADD RANDOMNESS HERE
        sorted_diff_scores = sorted(diff_scores.items(), key=lambda x:x[1], reverse=False)
        converted_dict = dict(sorted_diff_scores)
        # Define your score range
        min_score = next(iter(converted_dict.values()))  # Lowest score
        max_score = min_score + DIFF_RANGE
        # Filter indexes within the score range
        indexes_within_range = [index for index, score in converted_dict.items() if min_score <= score <= max_score]

        if list(set(indexes_within_range).difference(set(self.used_indexes))) != []:
            indexes_within_range = list(set(indexes_within_range).difference(set(self.used_indexes)))
            # print(f"[DEBUG] removed used indexes")

        # Get a random index from the filtered indexes
        random_index = random.choice(indexes_within_range)
        # print(f"[DEBUG] random_index: {random_index} ({converted_dict[random_index]})")
        # print(f"[DEBUG] best_fit_tile_index: {best_fit_tile_index} ({min_diff})")
        self.used_indexes += [random_index]
...
def build_mosaic(result_queue, all_tile_data_large, original_img_large):
    ...
    while True:
        try:
            ...
            if img_coords == EOQ_VALUE:
                ...
                if best_fit_tile_index in tiles_dist.keys():
                    tiles_dist[best_fit_tile_index] += 1
                else:
                    tiles_dist[best_fit_tile_index] = 1
    ...
    print(f"Tiles distribution ({sum(tiles_dist.values())} tiles, with usage: {len(tiles_dist.keys())}/{len(all_tile_data_large)}): {tiles_dist}")
    print(f"[DEBUG] mosaic size: {mosaic.image.size}")
    print(f"[DEBUG] original_img_large size: {original_img_large.size}")
    if original_img_large.mode != 'RGBA':
        original_img_large = original_img_large.convert('RGBA')
    overlay = Image.new('RGBA', mosaic.image.size, (0, 0, 0, 0))
    overlay = Image.blend(overlay, original_img_large, alpha=(TRANSPARENCY / 255))
    mosaic.image.paste(overlay, (0, 0), overlay)
...
def compose(original_img, tiles):
    ...
    try:
        ...
        Process(
            target=build_mosaic, args=(result_queue, all_tile_data_large, original_img_large)
        ).start()
